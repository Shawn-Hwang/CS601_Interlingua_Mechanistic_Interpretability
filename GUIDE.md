...
```python
MODEL_NAME = "gemma-2-2b"          # default, ~5GB VRAM
...
```python
N_SAMPLES_DIRECTION = 10   # was 200
...
**What it does:** Extracts a linear "English direction" from the residual stream at each layer using parallel EN/ES/FR/ZH sentence pairs from FLORES-200.
...
2. Loads FLORES-200 parallel sentences for each language pair (EN-ES, EN-FR, EN-ZH)
3. For each pair:
   ...
   - Averages activations over content-word token positions (excluding BOS/EOS/padding)
   - Computes the English direction at each layer via two methods:
     - **Mean difference**: `d = mean(EN) - mean(Other)`, normalized
     - **Logistic regression**: weight vector of an EN-vs-Other classifier, normalized
   - Computes cosine similarity between the two methods at each layer (convergence check)

**Outputs (in `results/directions/`):**
- `english_direction_{lang}_L{layer}_{method}.pt` — direction vector per layer per method
- `convergence_{lang}.json` — cosine similarities between the two methods

**What to check:**
- Cosine similarity between mean_diff and logreg should be high (>0.5) at middle layers
- If similarity is low everywhere, the direction may not be well-defined for this model
- Compare across languages: if EN-ES, EN-FR, EN-ZH directions are similar, the direction captures "English vs. non-English" rather than "English vs. Spanish"

**Expected runtime:** 30–60 min on GPU, longer on CPU.
...
3. **`convergence.png`** — Cosine similarity between mean_diff and logreg across layers
   - One line per language pair
   - Validates that the two extraction methods agree

4. **`mean_projection_profile.png`** — Average English-direction projection at each layer for Spanish input
...
├── utils.py                        # Shared utilities (model loading, hooks, generation)
├── phase1_extract_direction.py     # Phase 1: direction extraction
...
└── results/
    ├── directions/                 # Phase 1 outputs (direction vectors .pt)
...
|-----------|---------|-----------------|
| `MODEL_NAME` | `"gemma-2-2b"` | Which model to use |
| `N_SAMPLES_DIRECTION` | `200` | Sentence pairs for direction extraction |
...
| `DIRECTION_METHOD` | `"mean_diff"` | Primary direction method for ablation |
